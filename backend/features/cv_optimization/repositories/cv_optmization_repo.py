...
class CVOptRepository:
    ...
    async def get_optmization_reports_by_user(self, user_id: str) -> list[Dict]:
        ...
        db = await self._get_client()
        response = await db.table("cv_optimization_reports").select("*, cv_optimization_requests!inner(user_id), cv!left(parsed_content ->> title, cv_layout_analysis ->> original_filename), job_postings!left(parsed_data ->> job_title)").eq("cv_optimization_requests.user_id", user_id).order("generated_at", desc=True).execute()
        return response.data if response.data else []
