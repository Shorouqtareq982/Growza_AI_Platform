...
# =====================================================
# LEVEL DETECTOR (LLM + Rules)
# =====================================================

class LevelDetector:
    """
    Hybrid level detection using LLM + rule-based validation
    """
    ...
    def detect_level(
        ...
        track_skills_count: int
    ) -> Dict[str, Any]:
        """
        Detect user level with confidence
        ...
        """
        logger.info("Detecting user level...")
        
        # Extract features
        experience_years = parsed_cv_data.get('years_of_experience') or \
                          self._estimate_experience(parsed_cv_data)
        
        job_titles = parsed_cv_data.get('job_titles', [])
        projects = parsed_cv_data.get('projects', [])
        education = parsed_cv_data.get('education', '')
        
        # Try LLM detection first
        try:
            llm_result = self._detect_with_llm(
            ...
            if llm_result:
                # Validate with rules
                validated_result = self._validate_with_rules(
                    llm_result,
                    experience_years,
                ...
                validated_result['fallback_used'] = False
    ...
    def _detect_with_llm(
        ...
        """Use LLM for level detection"""
        
        prompt = LEVEL_DETECTION_PROMPT.format(
            experience_years=experience_years or 'Unknown',
            job_titles=', '.join(job_titles[:5]) if job_titles else 'None',
            skills=', '.join(skills[:15]) if skills else 'None',
    ...
    def _validate_with_rules(
        ...
        """Validate LLM result with hard rules"""
        
        detected_level = llm_result.get('detected_level', 'beginner')
        confidence = llm_result.get('confidence', 0.5)
        
        # Rule validation
        skill_coverage = skill_count / track_skills_count if track_skills_count > 0 else 0
        
        # Adjust confidence based on rules
        if detected_level == 'advanced' and skill_coverage < 0.5:
            # LLM says advanced but low skill coverage - reduce confidence
            confidence *= 0.7
        
        if detected_level == 'beginner' and experience_years and experience_years > 5:
            # LLM says beginner but 5+ years exp - suspicious
            confidence *= 0.6
    ...
    def _rule_based_detection(
        ...
        if any(kw in titles_text for kw in ['senior', 'lead', 'principal', 'architect']):
            score += 2
        elif any(kw in titles_text for kw in ['junior', 'intern', 'trainee']):
            score -= 1
        ...
        return {
            ...
            'reasoning': f'Rule-based: score={score}, exp={experience_years}y, coverage={skill_coverage:.0%}'
        }
    
    def _estimate_experience(self, parsed_cv_data: Dict) -> Optional[int]:
        """Estimate years of experience from CV data"""
        experience = parsed_cv_data.get('experience', [])
        
        if not isinstance(experience, list) or not experience:
            return None
        
        # Rough estimate: 2 years per job
